...
void main() {
    ...
    const vec2 center = vec2(16, 16);

    float modX = mod(gl_FragCoord.x, res.x/4);
    float modY = mod(gl_FragCoord.y, res.y/1);
    vec2 modFragCoord = vec2(modX, modY);
    float dist = distance(modFragCoord, center);
    ...
    float radius = 3*(pow(amplitude*20, 2));
    
    //bool lit = bool(gl_FragCoord.y < displayMag);
    bool lit = bool(ceil(dist) < radius);
    bool fftLit = bool(gl_FragCoord.y < band*res.y);

    //diffuseColor = vec4(band, lit ? gl_FragCoord.y/res.y : 0, lit ? gl_FragCoord.y/res.y : 0, 1.0);
    diffuseColor = vec4((lit || fftLit) ? 1 : 0, lit ? 1 : 0, lit ? 1 : 0, 1.0);
}